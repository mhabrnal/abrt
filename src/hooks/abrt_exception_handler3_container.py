...
def write_dump(tb_text, tb):
    ...
    data["reason"] = tb_text.splitlines()[0]
    data["backtrace"] = tb_text

    import json
    try:
        json_str='{{"ABRT": {0}}}\n'.format(json.dumps(data))
        call(["abrt-container-logger", json_str])
...
def handle_exception(etype, value, tb):
    ...
    try:
        ...
        '''
        # DO NOT CATCH PYTHONi EXCEPTION FROM INTERACTIV MODE
        if not sys.argv[0] or sys.argv[0][0] == "-":
            einfo = "" if not sys.argv[0] else " (python {0} ...)".format(sys.argv[0])
            log("detected unhandled Python exception in 'interactive mode{0}'"
                   .format(einfo))
            raise Exception
        '''
